#!/usr/bin/env python3
"""
Codex-powered code analyzer for GitHub PRs.
...
def load_config():
    ...
    config_path = Path(__file__).parent / 'codex_config.yml'
...
def analyze_with_codex(client, prompt, diff_content, files_content, config):
    """Send analysis request to OpenAI Codex."""
...
"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=config['codex'].get('temperature', 0.0),
            messages=[
                {
                    "role": "user",
                    "content": full_prompt
...
def parse_analysis_result(response_text):
    """Parse Codex's response to extract key findings."""
